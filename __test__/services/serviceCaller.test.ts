...
import { serviceCaller } from "../../src/services/serviceCaller";
...
describe("serviceCaller", () => {
  test("should thrown an error if could not reach the server", async () => {
    ...
    const caller = async () =>
      await serviceCaller(apiRoutes.index, {}, z.string());
  ...
  test("should return raw error string when receiving status response different than 200", async () => {
    ...
    const response = await serviceCaller(apiRoutes.index, {}, z.number());
  ...
  test("should return a successfully parsed response", async () => {
    ...
    const response = await serviceCaller(apiRoutes.index, {}, z.string());
